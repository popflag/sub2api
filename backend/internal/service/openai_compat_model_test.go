...
func TestForwardAsAnthropic_TrimsFullReplayOnlyForCodexCompatModels(t *testing.T) {
	...
	require.Contains(t, gjson.GetBytes(codexBody, "input.0.content.0.text").String(), "<todo-guard>")
...
func TestForwardAsAnthropic_OAuthCompatKeepsFullReplayForCacheGrowth(t *testing.T) {
	...
	require.Contains(t, gjson.GetBytes(upstream.lastBody, "input.0.content.0.text").String(), "<todo-guard>")
...
func TestForwardAsAnthropic_ReplaysFullToolHistoryWhenPreviousResponseUnavailable(t *testing.T) {
	...
	require.Contains(t, gjson.GetBytes(upstream.bodies[1], "input.0.content.0.text").String(), "<todo-guard>")
...
func TestForwardAsAnthropic_OAuthAddsClaudeCodeTodoGuardForCompatModel(t *testing.T) {
	...
	require.Contains(t, gjson.GetBytes(upstream.lastBody, "input.1.content.0.text").String(), "<todo-guard>")
	...
	for _, message := range []string{
		...
		t.Run(message, func(t *testing.T) {
			...
			for _, sent := range upstream.bodies[1:] {
				...
				require.Contains(t, gjson.GetBytes(sent, "input.0.content.0.text").String(), "<todo-guard>")
